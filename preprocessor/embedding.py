...
#    @log_time_delta
    def get_embedding(self, dataset_name, fname = None,language ="en", fresh = True):
        pkl_name="tmp/"+dataset_name+".subembedding.pkl"
...
#            fname= "embedding/embedding.200.header_txt"
    ...
    def get_subVectors(self,vectors,dim = 300):
        ...
        count = 1
        import codecs
        with codecs.open("tmp/oov.txt","w",encoding="utf-8") as f:
            for word in vocab:
                if word in vectors:
                    count += 1
...
#       Force the embedding of 'UNK' to be all zeros
        embedding[0,:] = 0
        return embedding
...
#    @log_time_delta
    ...
    def text_to_sequence(self,sentence):    
        ...
        tokens = sentence[:self.max_sequence_length]   # tokens = [w for w in tokens if w not in stopwords.words('english')]
        seq = [self.dictionary[str(w)] if w in self.dictionary else self.dictionary['UNK'] for w in tokens]
        return seq
