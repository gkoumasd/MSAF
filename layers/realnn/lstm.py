...
class LSTM(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, layer=1, device=None):
        ...
        self.device = device
        if layer <= 0:
            print("Error: The number of layer should be a positive integer, but got [{}] instead.".format(str(layer)))

        self.add_module("layer_1", LSTMLayer(self.i_dim, self.h_dim))
        for i in range(1, self.layer):
            self.add_module("layer_{}".format(i + 1), LSTMLayer(self.h_dim, self.h_dim,self.device))

    def forward(self, inputs, hc_0=None):
        batch = inputs.size()[0]
        if hc_0 == None:
            h, c = torch.zeros(batch, self.layer, self.h_dim).to(self.device), torch.zeros(batch, self.layer, self.h_dim).to(self.device)
        else:
            h = hc_0[0].clone()
            c = hc_0[1].clone()

        for i in range(self.layer):
            inputs, hc_n = self.__getattr__("layer_{}".format(i + 1))(inputs, list(map(lambda x:x[:, i, :].squeeze(), [h, c])))
            if i == 0:
...
class LSTMLayer(nn.Module):
    ...
    def forward(self, inputs, hc_0 = None):
        ...
        if hc_0 == None:
            h, c = torch.zeros(batch, self.h_dim).to(self.device), torch.zeros(batch, self.h_dim).to(self.device)
        else:
            h = hc_0[0].clone().to(self.device)
            c = hc_0[1].clone().to(self.device)
