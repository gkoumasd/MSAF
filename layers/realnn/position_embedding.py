...
def make_positions(tensor, padding_idx, left_pad):
    ...
    """
    max_pos = padding_idx + 1 + tensor.size(1)
    new_tensor = tensor.new()
    if new_tensor.numel() < max_pos:
        torch.arange(padding_idx +1, max_pos, out=new_tensor)
        
    mask = tensor.ne(padding_idx)
    positions = new_tensor[:tensor.size(1)].expand_as(tensor).long()
    if left_pad:
        positions = positions - mask.size(1) + mask.long().sum(dim=1).unsqueeze(1)
    tensor_copy = tensor.clone()
    return tensor_copy.masked_scatter_(mask, positions.float()[mask]).long()
